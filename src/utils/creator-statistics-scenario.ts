...
import { isNoContractCreator } from './creator-registration-mode';
...
const hasNoContractPaymentWorkflow = (user?: StatisticsScenarioUser | null) =>
  user?.documentWorkflowStates?.some((state) => state.campaign?.key === NO_CONTRACT_PAYMENT_CAMPAIGN_KEY);

export const isMarchAprilStatisticsScenario = (user?: StatisticsScenarioUser | null) =>
  Boolean(
    isNoContractCreator(user) ||
      hasNoContractPaymentWorkflow(user) ||
